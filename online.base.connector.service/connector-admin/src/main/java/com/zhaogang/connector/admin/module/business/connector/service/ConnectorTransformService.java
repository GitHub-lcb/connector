...
import com.zhaogang.connector.admin.module.business.connector.util.RsaUtils;
...
public class ConnectorTransformService {
    ...
    public Object transform(Object data, Map<String, Object> config, String scriptType, String scriptContent) {
        Object result = data;
        ...
        // 4. 执行自定义脚本 (Groovy/Python)
        if (scriptType != null && scriptContent != null && !scriptContent.trim().isEmpty()) {
            result = processScript(result, scriptType, scriptContent);
        }
    ...
    public Object transform(Object data, Map<String, Object> config) {
        return transform(data, config, null, null);
    }

    private Object processScript(Object data, String type, String content) {
        if ("groovy".equalsIgnoreCase(type)) {
            try {
                Binding binding = new Binding();
                binding.setVariable("data", data);
                // binding.setVariable("log", log); // Need @Slf4j or logger
            ...
            throw new RuntimeException("Python script execution not supported yet.");
        }
        return data;
    }
