...
import com.zhaogang.connector.admin.module.business.connector.service.ConnectorTransformService;
...
public class ConnectorProxyController {
    ...
    public ResponseEntity<Object> handleRequest(HttpServletRequest request, @RequestBody(required = false) Object body) {
        ...
        try {
            // 步骤 2：根据映射配置转换请求数据
            Object transformedBody = connectorTransformService.transform(body, route.getMappingConfig(), route.getScriptType(), route.getScriptContent());

            // Check if forwarding is enabled (Default: true)
            if (Boolean.FALSE.equals(route.getForwardFlag())) {
                ...
                logRequest(route.getId(), route.getChannel(), path, 200, latency, null, originalParams, transformedParams, transformedParams);
                
                log.info("转发已禁用，直接返回转换结果, routeName: {}", route.getName());
                return ResponseEntity.ok(transformedBody);
            }
